...
if (info == null) {
    Velocity newV = this.velocity.setByDt(dt);
...
Velocity v2 = this.velocity.setByDt(dt);
Point tmp = v2.applyToPoint(this.point);
Point start = new Point((int) this.point.getX(), this.point.getY());
Point end = new Point((int) tmp.getX(), tmp.getY());
Line trajectory = new Line (start, end);
